//
//  chain3swift_personal_signature_Tests.swift
...
class RinkebyPersonalSignatureTests: XCTestCase {
    func testPersonalSignature() throws {
        ...
        let signature = try chain3.personal.signPersonalMessage(message: message, from: expectedAddress, password: "")
        ...
        try! print("Personal hash = " + Chain3Utils.hashPersonalMessage(message).toHexString())
        let signer = try chain3.personal.ecrecover(personalMessage: message, signature: signature)
    ...
    func testPersonalSignatureOnContract() throws {
        ...
        let expectedAddress = keystoreManager.addresses[0]
        print(expectedAddress)
        let signature = try chain3.personal.signPersonalMessage(message: messageData, from: expectedAddress, password: "")
        let unmarshalledSignature = try SECP256K1.unmarshalSignature(signatureData: signature)
        print("V = " + String(unmarshalledSignature.v))
        print("R = " + Data(unmarshalledSignature.r).toHexString())
        print("S = " + Data(unmarshalledSignature.s).toHexString())
        try print("Personal hash = " + Chain3Utils.hashPersonalMessage(messageData).toHexString())
        ...
        let contract = try chain3.contract(jsonString, at: "0x6f1745a39059268e8e4572e97897b50e4aab62a8")
        ...
        let hash2 = try Chain3Utils.hashPersonalMessage(messageData)
